<template>
    <v-container>
        ...
        <div class="panel">
            ...
            <PermissionList @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></PermissionList>
            <UserAccessProfile @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></UserAccessProfile>
            <AccessControlList @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></AccessControlList>
            <UserPermissions @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></UserPermissions>
                ...
                <v-table>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap" label="RevokedAt">{{ val.revokedAt }}</td>
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">Authorization 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">Authorization 수정</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                        <div>
                            <Number label="AuthorizationId" v-model="selectedRow.authorizationId" :editMode="true"/>
                            <String label="Role" v-model="selectedRow.role" :editMode="true"/>
                            <String label="PermissionType" v-model="selectedRow.permissionType" :editMode="true"/>
                            <Date label="GrantedAt" v-model="selectedRow.grantedAt" :editMode="true"/>
                            <Date label="RevokedAt" v-model="selectedRow.revokedAt" :editMode="true"/>
                            ...
                            <AuthorizationStatus offline label="Status" v-model="selectedRow.status" :editMode="true"/>
                            <AuthorizationLevel offline label="Level" v-model="selectedRow.level" :editMode="true"/>
