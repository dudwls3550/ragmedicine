        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:140px;">
                    <Number class="attributes-list" label="LogRetentionPolicyId" v-model="value.parameters.logRetentionPolicyId" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        name: 'RetentionPolicyConfiguration',
        components:{
        },
        props: {},
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'logRetentionPoliciesauditLogs/search/retentionPolicyConfiguration',
                parameters: {}
            },
        }),
        created() {
