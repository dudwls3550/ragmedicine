        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:140px;">
                    <Number class="attributes-list" label="ResidentId" v-model="value.parameters.residentId" :editMode="editMode"/>
                </v-col>
                <v-col style="max-width:140px;">
                    <Number class="attributes-list" label="QuestionAnswerId" v-model="value.parameters.questionAnswerId" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'feedbacksfeedbacks/search/feedbackSummary',
                parameters: {}
            },
        }),
        created() {
